...
describe('validateContactRequest', () => {
  ...
  describe('name', () => {
    ...
    it('passes when name is exactly 200 characters', () => {
      expect(() => validateContactRequest({ ...valid, name: 'a'.repeat(200) })).not.toThrow();
    });
  });
  ...
  describe('email', () => {
    ...
    it('throws when email has no @', () => {
      expect(() => validateContactRequest({ ...valid, email: 'notanemail.com' })).toThrow(
        'Invalid email',
      );
    });

    it('throws when email has no domain part', () => {
      expect(() => validateContactRequest({ ...valid, email: 'user@' })).toThrow('Invalid email');
    });

    it('throws when email has no TLD', () => {
      expect(() => validateContactRequest({ ...valid, email: 'user@domain' })).toThrow(
        'Invalid email',
      );
    });
    ...
    it('passes for valid email formats', () => {
      const emails = ['user@example.com', 'user+tag@sub.example.org', 'a@b.co'];
  ...
  describe('message', () => {
    ...
    it('passes when message is exactly 2000 characters', () => {
      expect(() => validateContactRequest({ ...valid, message: 'a'.repeat(2000) })).not.toThrow();
    });
  });
});
